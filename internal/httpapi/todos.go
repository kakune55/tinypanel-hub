...
func (s *Server) handlePostTodo(w http.ResponseWriter, r *http.Request) {
	...
	user, _ := currentUser(r)
	todo, err := s.services.Todos.Create(user.ID, text, req.Status)
	if err != nil {
...
func (s *Server) handleGetTodo(w http.ResponseWriter, r *http.Request) {
	...
	user, _ := currentUser(r)
	todo, found := s.services.Todos.Get(user.ID, id)
	if !found {
...
func (s *Server) handlePatchTodo(w http.ResponseWriter, r *http.Request) {
	...
	user, _ := currentUser(r)
	todo, found, swapped, err := s.services.Todos.Update(user.ID, id, req.Version, patch)
	if err != nil {
		s.writeStoreError(w, err)
		return
	}
...
func (s *Server) handleDeleteTodo(w http.ResponseWriter, r *http.Request) {
	...
	user, _ := currentUser(r)
	found, swapped, err := s.services.Todos.Delete(user.ID, id, req.Version)
	if err != nil {
		s.writeStoreError(w, err)
		return
	}
